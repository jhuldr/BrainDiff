...
`coords [N, 3]`, not a dense volume. Tokenizing is pure CPU work (SimpleITK
resample dominates at ~1.1 s/volume), so it belongs in the worker processes and
leaves the GPU doing only the ViT forward.

Deliberately NOT a reuse of VJEPAVolumePrepd: that picks `target_depth` slices by
`torch.linspace` index selection, which is frame-sampling for a video encoder and
makes the anatomy under token i depend on each volume's native depth.

Two upstream defaults must be overridden, both verified:

1. `remove_background=True` drops a token if ANY voxel in its 4x16x16 patch falls
   below a *per-volume* 10th-percentile threshold. Four series of one subject gave
   1034 / 1029 / 1034 / 1034 tokens — unequal counts and unequal index sets, which
   makes a token-wise ref/main difference meaningless. Off here, so every volume
   yields the full dense grid.
2. `prepare_for_inference` rescales each volume by its own percentile clip and
   min-max. Two timepoints then get different affine maps and the difference mixes
   anatomy with intensity rescaling. We use one fixed affine for every volume.

Geometry is NeuroVFM's, not ours: 1x1x4 mm (`neurovfm/data/utils.py:200-202`),
patches (4,16,16). Our 193x229x193 @ 1 mm template volumes land on
[D,H,W] = (48,224,192) -> a 12x14x12 = 2016 token grid at ~16 mm per token.
"""
import numpy as np
import torch
...
def to_neurovfm_grid(volume, axcodes):
    ...
    Upstream does this with SimpleITK: reorient to RPI, BSpline-resample the
    slice axis 1 mm -> 4 mm, centre-crop to patch multiples. That costs 460 ms
    per volume and is 89% of the preprocessing budget -- 62.7 core-hours per S1
    epoch, which is unaffordable.

    It is also more machinery than this data needs. Every volume here is already
    template-space with an identical affine, so the resample is one FIXED
    downsample rather than a general one: flip, mean-pool groups of 4, crop.

    RAS -> RPI flips BOTH the A and S axes. Omitting them was worth 0.58 voxel
    correlation against the reference; including them gives 0.9935, with the
    residual being BSpline vs mean-pool interpolation.
    """
...
def neurovfm_geometry(orig_shape):
    """Per-axis (flip, offset, extent) taking a native RAS voxel index to the
    NeuroVFM grid, in output axis order (D, H, W).

    THE single source of truth for how a volume maps into the preprocessed image,
    shared by `to_neurovfm_grid` (which builds the image) and
    `voxel_box_to_percent` (which places box coordinates in it). These must derive
    from the same formulas: computing them separately in separate files is exactly
    how the V-JEPA coordinates silently stopped matching the images they annotate.

    Returns a list of (native_axis, flip, offset, extent) in D, H, W order:
...
def voxel_box_to_percent(box, orig_shape=(193, 229, 193)):
    ...
    Input is (x1, x2, y1, y2, z1, z2) as produced by
    dataset3D/ExtractData/extract_meta_positional.py:get_bounding_box -- raw
    np.argwhere min/max indices in native axis-major order (x=R, y=A, z=S).

    Output is each coordinate's position in the FINAL preprocessed image as a
    percentage of that image's extent, so the model needs no resample affine.

    This is NOT a relabelling of the V-JEPA version. That one mapped x->D, y->H,
    z->W with no flips; here D=S and W=R, so the first and third axes swap, and
    the RAS->RPI reorientation mirrors two of them. A box normalized under the
    old convention is wrong by up to 66 percentage points once off-centre.
    """
...
class NeuroVFMGridd(MapTransform):
    ...
    Kept separate from tokenization so augmentation still has a spatial volume to
    work on: the pipeline is grid -> augment -> tokenize. Augmenting after
    tokenization would mean warping a flat token list, which is not the same
    operation. Running the augmentations at 4 mm rather than 1 mm is also ~64x
    cheaper and loses nothing, since the model never sees the finer grid.
    """
...
def stack_modalities(per_modality, n_tokens):
    """[{tokens, coords} or None] * M -> (tokens [M,N,1024], coords [M,N,3],
    present [M]).

    Shapes are uniform because every volume is template-space, which is what lets
    the encoder pack a batch and reshape the output back to [B, N, D].
    """
    m = len(per_modality)
